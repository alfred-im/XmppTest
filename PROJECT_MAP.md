...
**Non deducibile — multi-account client**: `AccountManager` / `AccountSession` — ogni account aperto ha client Supabase dedicato (`SharedPreferencesLocalStorage` per `userId`), `InboxController` sempre attivo con realtime. Lista `OpenAccount` in storage = account autenticati (non bookmark). Switch = `setFocus` senza `setSession`. **Chat aperta** (`HomeScreen._activePeer`) si resetta al cambio `focusUserId`. Coda invio: chiave `userId|peerProfileId`. Overlay credenziali semi-trasparente su `HomeScreen`. Doc: `docs/decisions/multi-account-parallel-sessions.md`, `docs/design/auth-overlay-shell.md`, `docs/implementation/multi-account-client.md`.
