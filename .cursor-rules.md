...
0. **NON Modificare Senza Conferma** - Analisi sì, scrittura solo con ok esplicito (CRITICO - RISPARMIO TOKEN)
...
| **Scrittura** (modifica persistente) | Edit file, nuova doc nel repo, migrazioni, SQL live, commit, push | **Sempre sì** — comando esplicito o risposta affermativa a «Vuoi che proceda con le modifiche?» |
...
- ✅ **Chiedere conferma prima di ogni scrittura**: "Vuoi che proceda con le modifiche?"
...
### Comandi espliciti che autorizzano le modifiche:
L'utente deve dire qualcosa come:
- "Procedi" / "Implementa" / "Sviluppa"
- "Fai le modifiche" / "Correggi il problema"
- "Vai avanti" / "Ok, fallo"
- Risposta affermativa esplicita a «Vuoi che proceda con le modifiche?»
...
```
UTENTE descrive problema O chiede analisi
    ↓
Se solo descrizione → discuto / propongo indagine
    ↓
Se analisi richiesta o accettata → leggo codice/docs, diagnostico (SENZA modifiche)
    ↓
Comunico esito / cause / opzioni
    ↓
Chiedo: "Vuoi che proceda con le modifiche?"
    ↓
UTENTE: "Sì" / "Procedi" / "Implementa" / "Ok, fallo"
    ↓
...
- ✅ **Chiedere conferma prima di modificare file**: «Vuoi che proceda con le modifiche?»
...
- ✅ Dopo l'analisi: chiedere «Vuoi che proceda con le modifiche?»
- ✅ Aspettare comando esplicito per la scrittura: «Procedi», «Implementa», «Ok, fallo», ecc.
...
**Ricorda: analisi sì, modifiche solo con conferma. Completare un task = rispettare queste regole. La Mappa del Progetto è il CONTRATTO. Il Metodo Analitico è la base di tutto. Debug = causa, non scelta strutturale autonoma né permesso di scrivere. In caso di dubbio tra regole, vince Principio di Certezza e accordo utente sulle decisioni strutturali.**
