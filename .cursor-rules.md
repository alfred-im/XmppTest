...
**FONDAMENTALE: All'inizio di OGNI task, genera o aggiorna e ricarica da zero una mappa completa e aggiornata del progetto.**

Questa è la regola più importante per stabilizzare la memoria e creare un "punto di verità" che il modello rilegge ad ogni inizio.

### Regole fondamentali:
- ✅ **SEMPRE leggere `/workspace/PROJECT_MAP.md` all'inizio di ogni task**
- ✅ **SEMPRE verificare se la mappa è aggiornata rispetto al codice corrente**
- ✅ **SEMPRE aggiornare la mappa se ci sono stati cambiamenti significativi**
- ✅ **La mappa deve essere completa e dettagliata**
- ✅ **La mappa è il "punto di verità" da cui partire per ogni analisi**
- ❌ **NON iniziare MAI un task senza aver letto la mappa del progetto**
- ❌ **NON fare assunzioni sul progetto senza verificare con la mappa**
...
1. **Leggere la mappa**:
   ```bash
   # SEMPRE come primo step
   Read /workspace/PROJECT_MAP.md
   ```

2. **Verificare aggiornamento**:
   - Controllare data ultima revisione
   - Verificare se ci sono stati cambiamenti significativi da allora
   - Controllare se le informazioni sono ancora accurate

3. **Aggiornare se necessario**:
   - Se ci sono stati cambiamenti significativi, aggiornare la mappa
   - Aggiornare data di ultima revisione
   - Documentare i cambiamenti principali

4. **Utilizzare come punto di verità**:
   - Basare TUTTE le analisi sulla mappa
   - Non fare assunzioni sul progetto
   - Riferirsi sempre alla mappa per decisioni architetturali

### Quando aggiornare la mappa:

Aggiorna la mappa quando:
- ✅ Aggiungi nuove dipendenze npm
- ✅ Crei nuovi file/directory significativi
- ✅ Modifichi l'architettura o i layer
- ✅ Aggiungi nuovi servizi esterni
- ✅ Modifichi schema database
- ✅ Implementi nuove feature importanti
- ✅ Risolvi bug architetturali
- ✅ Modifichi configurazione build
...
1. **Riduce ricostruzioni errate**: Avere un documento di riferimento completo evita di fare assunzioni sbagliate
2. **Stabilizza la memoria**: Punto di verità che viene riletto ad ogni task
3. **Accelera l'analisi**: Non serve ricostruire la struttura ogni volta
4. **Migliora la coerenza**: Tutte le decisioni partono dalla stessa base
5. **Facilita onboarding**: Nuovo contesto o nuova sessione parte da documento completo
