...
0. **NON Sviluppare Automaticamente** - Aspettare comando esplicito (CRITICO - RISPARMIO TOKEN)
...
## NON Sviluppare Automaticamente - Aspettare comando esplicito

**CRITICO: NON iniziare MAI a sviluppare codice automaticamente quando l'utente spiega un problema. ASPETTARE sempre un comando esplicito.**

Questa è la regola più importante per evitare sprechi di token causati da sviluppo prematuro e successivi rollback.

### Il problema che risolve:
Quando l'utente spiega un problema, NON vuole necessariamente che io inizi subito a sviluppare. Spesso vuole:
...
Se inizio a sviluppare automaticamente, poi devo fare rollback, sprecando token che l'utente paga.

### Regole fondamentali:
- ❌ **NON iniziare MAI a sviluppare quando l'utente descrive un problema**
- ❌ **NON leggere file, fare modifiche, o eseguire comandi di sviluppo senza comando esplicito**
- ❌ **NON assumere che "spiegare un problema" significhi "implementa la soluzione"**
- ✅ **ASPETTARE sempre che l'utente dica esplicitamente di procedere con lo sviluppo**
- ✅ **Considerare OGNI conversazione come discussione finché l'utente non dice altrimenti**
- ✅ **Chiedere conferma prima di procedere: "Vuoi che proceda con l'implementazione?"**
- ✅ **Posso leggere la PROJECT_MAP.md per contestualizzare, ma NON altri file senza comando esplicito**

### Comandi espliciti che autorizzano lo sviluppo:
...
### Cosa posso fare SENZA comando esplicito:
- Rispondere a domande
- Spiegare concetti
- Analizzare il problema teoricamente
- Suggerire approcci e soluzioni
- Discutere pro e contro
- Leggere SOLO la PROJECT_MAP.md per avere contesto
- Chiedere chiarimenti

### Cosa NON posso fare SENZA comando esplicito:
- Leggere file di codice
- Modificare file
- Eseguire comandi
- Fare grep/search nel codice
- Iniziare l'implementazione
- Creare o modificare documentazione
...
UTENTE descrive problema
    ↓
IO ascolto e capisco
    ↓
IO spiego/discuto/analizzo (SENZA leggere codice)
    ↓
IO chiedo: "Vuoi che proceda con l'implementazione?"
    ↓
UTENTE dice esplicitamente "Sì" / "Procedi" / "Implementa"
    ↓
SOLO ORA inizio a sviluppare (leggere codice, modificare, etc.)
```

### Esempio di conversazione corretta:

**SBAGLIATO:**
```
Utente: "Ho un problema con il login, a volte non funziona"
Io: [legge 10 file, analizza codice, fa modifiche]
Utente: "Aspetta, volevo prima capire perché succede"
[SPRECO DI TOKEN - ROLLBACK NECESSARIO]
```

**CORRETTO:**
```
Utente: "Ho un problema con il login, a volte non funziona"
Io: "Capisco. Potrebbe essere legato a [spiegazione teorica]. 
     Vuoi che analizzi il codice per identificare la causa esatta e procedere con una correzione?"
Utente: "Sì, procedi"
Io: [ORA leggo codice, analizzo, modifico]
```

### Integrazione con altre regole:

- Questa regola ha PRIORITÀ MASSIMA su tutte le altre
- Anche se le altre regole dicono di leggere la mappa, analizzare il codice, etc., PRIMA devo aspettare il comando esplicito
- L'unica eccezione è la lettura della PROJECT_MAP.md per avere contesto generale
...
7. **Stato Build** (solo configurazioni NON standard):
   ...
   - Script npm con logiche PARTICOLARI
...
| 4 | **Debug e Testing** (questa sezione) | *Come* trovare la causa senza coinvolgere l'utente nel debug manuale |
...
**La fix non fa parte di questa sezione.** Implementare, scegliere tra alternative architetturali e dichiarare una soluzione «corretta» richiedono comando esplicito e, per decisioni strutturali, accordo dell'utente.
...
- ✅ **NON fare `git commit` / `git push` senza `verify_ok`** — il gate locale deve passare prima del push (la CI blocca `deploy-alpha` altrimenti)
...
Codice storico solo su tag git `legacy/web-client-final` — non documentato in questo repository.
...
### 0. NON Sviluppare Automaticamente (CRITICO - PRIMO CONTROLLO)
**PRIMA DI OGNI ALTRA COSA:**
- ❌ NON iniziare automaticamente a sviluppare quando l'utente descrive un problema
- ✅ Considerare ogni conversazione come discussione finché l'utente non dice esplicitamente di procedere
- ✅ Posso discutere, spiegare, analizzare teoricamente
- ✅ Posso leggere SOLO la PROJECT_MAP.md per contestualizzare
- ✅ Chiedere: "Vuoi che proceda con l'implementazione?"
- ✅ Aspettare comando esplicito: "Procedi", "Implementa", "Sviluppa", etc.
- **SOLO DOPO comando esplicito → procedere con gli step successivi**

### 1. Mappa del Progetto (PRIMO STEP DOPO COMANDO ESPLICITO - OBBLIGATORIO)
**Quando l'utente ha dato il comando di procedere:**
...
### 2b. Debug (dopo comando esplicito, prima della fix)
- Individuare e riprodurre la causa in autonomia (senza chiedere console/log all'utente)
- Comunicare la causa trovata
- **Stop**: non implementare la correzione finché approccio e modifiche non sono concordati (Principio di Certezza)
...
**Ricorda: NON sviluppare automaticamente - aspettare comando esplicito. La Mappa del Progetto è il CONTRATTO. Il Metodo Analitico è la base di tutto. Debug = solo causa, mai scelta strutturale autonoma. Ogni altra regola si applica attraverso il metodo analitico; in caso di dubbio tra regole, vince Principio di Certezza e accordo utente sulle decisioni strutturali.**
